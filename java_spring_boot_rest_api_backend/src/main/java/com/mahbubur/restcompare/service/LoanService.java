...
public class LoanService {
    ...
    public PageResult<LoanResponse> listActive(Long memberId, int page, int size) {
        Pageable pageable = pageRequest(page, size);
        int safePage = Math.max(page, 1);
        int safeSize = Math.min(Math.max(size, 1), 200);

        Page<LoanResponse> mapped;
        if (memberId != null) {
            memberRepository.findById(memberId)
                    .orElseThrow(() -> new ResourceNotFoundException("Member not found: " + memberId));
            mapped = loanRepository
                    .findByMember_IdAndStatus(memberId, Loan.LoanStatus.ACTIVE, pageable)
                    .map(this::toResponse);
        } else {
            mapped = loanRepository
                    .findByStatus(Loan.LoanStatus.ACTIVE, pageable)
                    .map(this::toResponse);
        }
    ...
    public LoanResponse renew(Long loanId, RenewRequest req) {
        ...
        LocalDate newDue = req.getDueDate();
