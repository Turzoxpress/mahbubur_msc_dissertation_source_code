...
export default function BackendSwitcher(props: {
  ...
  const presets = useMemo(
    () => [
      { label: 'Spring Boot (8090)', url: import.meta.env.VITE_SPRING_URL || 'http://localhost:8090' },
      { label: 'Django (8000)', url: import.meta.env.VITE_DJANGO_URL || 'http://localhost:8000' },
    ],
    [],
  )
  ...
  return (
    <Card>
      <CardHeader
        title="Backend"
        subtitle="Switch between Spring Boot and Django, then run the same API calls against both."
        right={
